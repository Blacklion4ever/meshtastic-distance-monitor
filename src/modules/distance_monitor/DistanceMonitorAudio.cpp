...
DistanceMonitorAudio::DistanceMonitorAudio() : concurrency::OSThread("DistanceMonitorAudio") {}
...
bool DistanceMonitorAudio::setup()
{
...
#else
    buzzerPin_ = config.device.buzzer_gpio != 0U ? static_cast<uint8_t>(config.device.buzzer_gpio) : 25U;
#endif
    ...
    noTone(buzzerPin_);
    initialized_ = true;
    LOG_INFO("Distance Monitor audio: buzzer ready gpio=%u config_gpio=%u",
             static_cast<unsigned>(buzzerPin_), static_cast<unsigned>(config.device.buzzer_gpio));
    setIntervalFromNow(DM_AUDIO_TICK_MS);
    return true;
}

void DistanceMonitorAudio::playDistanceBip() { startPattern(Pattern::DistanceBip); }
void DistanceMonitorAudio::playConfirmationBop() { startPattern(Pattern::ConfirmationBop); }
void DistanceMonitorAudio::playFaultBops() { startPattern(Pattern::FaultBops); }
void DistanceMonitorAudio::playPairingBops() { startPattern(Pattern::Pairing); }
void DistanceMonitorAudio::playTrackerNotification() { startPattern(Pattern::TrackerNotification); }
void DistanceMonitorAudio::playSearchEnter() { startPattern(Pattern::SearchEnter); }
void DistanceMonitorAudio::playSearchExit() { startPattern(Pattern::SearchExit); }
...
{
    return sosActive_ ||
           (pattern_ != Pattern::None && patternPriority(pattern_) > patternPriority(Pattern::SearchPulse));
}

void DistanceMonitorAudio::startPattern(Pattern pattern)
{
    if (!initialized_ || sosActive_) return;
    if (pattern_ != Pattern::None && patternPriority(pattern) < patternPriority(pattern_)) return;
    pattern_ = pattern;
    stepIndex_ = 0U;
    stepRunning_ = false;
}
...
int DistanceMonitorAudio::patternPriority(Pattern pattern) const
{
    switch (pattern)
    {
    case Pattern::TrackerNotification: return 7;
    case Pattern::Pairing: return 6;
    case Pattern::FaultBops: return 5;
    case Pattern::DistanceBip: return 4;
    case Pattern::ConfirmationBop: return 3;
    case Pattern::SearchEnter:
    case Pattern::SearchExit: return 2;
    case Pattern::SearchPulse: return 1;
    case Pattern::None:
    default: return 0;
    }
}

const DistanceMonitorAudio::Step *DistanceMonitorAudio::patternSteps(Pattern pattern, size_t &count) const
{
    switch (pattern)
    {
    case Pattern::DistanceBip:
        count = sizeof(DISTANCE_BIP_STEPS) / sizeof(DISTANCE_BIP_STEPS[0]); return DISTANCE_BIP_STEPS;
    case Pattern::ConfirmationBop:
        count = sizeof(CONFIRMATION_BOP_STEPS) / sizeof(CONFIRMATION_BOP_STEPS[0]); return CONFIRMATION_BOP_STEPS;
    case Pattern::FaultBops:
        count = sizeof(FAULT_BOPS_STEPS) / sizeof(FAULT_BOPS_STEPS[0]); return FAULT_BOPS_STEPS;
    case Pattern::Pairing:
        count = sizeof(PAIRING_STEPS) / sizeof(PAIRING_STEPS[0]); return PAIRING_STEPS;
    case Pattern::TrackerNotification:
        count = sizeof(TRACKER_NOTIFICATION_STEPS) / sizeof(TRACKER_NOTIFICATION_STEPS[0]); return TRACKER_NOTIFICATION_STEPS;
    case Pattern::SearchEnter:
        count = sizeof(SEARCH_ENTER_STEPS) / sizeof(SEARCH_ENTER_STEPS[0]); return SEARCH_ENTER_STEPS;
    case Pattern::SearchExit:
        count = sizeof(SEARCH_EXIT_STEPS) / sizeof(SEARCH_EXIT_STEPS[0]); return SEARCH_EXIT_STEPS;
    case Pattern::SearchPulse:
        count = dynamicSearchStepCount_; return dynamicSearchSteps_;
    case Pattern::None:
    default:
        count = 0U; return nullptr;
    }
}
...
void DistanceMonitorAudio::startTone(uint16_t frequencyHz)
{
    ...
    digitalWrite(BUZZER_EN_PIN, HIGH);
#endif
    if (frequencyHz == 0U) { stopTone(); return; }
...
{
    if (!initialized_) return DM_AUDIO_TICK_MS;
    if (sosActive_) { startTone(DM_BIP_FREQ_START_HZ); return DM_AUDIO_TICK_MS; }
    if (pattern_ == Pattern::None) return DM_AUDIO_TICK_MS;

    size_t stepCount = 0U;
    const Step *steps = patternSteps(pattern_, stepCount);
    if (steps == nullptr || stepIndex_ >= stepCount)
    {
        stopTone(); pattern_ = Pattern::None; stepRunning_ = false; return DM_AUDIO_TICK_MS;
    }
    ...
    {
        stopTone(); ++stepIndex_; stepRunning_ = false;
        if (stepIndex_ >= stepCount) pattern_ = Pattern::None;
    }
    return DM_AUDIO_TICK_MS;
}
