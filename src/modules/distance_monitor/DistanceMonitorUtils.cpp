...
namespace
{
float interpolate(float value, float x0, float y0, float x1, float y1)
{
    if (value <= x0)
        return y0;
    if (value >= x1)
...
void dmFormatBattery(uint8_t batteryPercent, char *buffer, size_t bufferSize)
{
    ...
    if (batteryPercent == DM_BATTERY_UNKNOWN)
        ...
        std::snprintf(buffer, bufferSize, "%u%%", static_cast<unsigned>(batteryPercent));
}
...
double dmCalculateDistanceMeters(double latitude1, double longitude1, double latitude2, double longitude2)
{
...
{
    return kind == DmPositionKind::FreshFix ? "FIX" : "NO_FIX";
}
